                    ...
                    <Card>
                        <CardHeader>
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div>
                                    ...
                                    <CardDescription>
                                        {relatedData.group ? `Группа: ${getRecordName(relatedData.group)}` : 'Группа не указана'}
                                    </CardDescription>
                                </div>
                        ...
                        <CardContent>
                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                ...
                                <InfoItem label="Дата изменения статуса" value={formatDate(student.status_changed_at)} />
...
function DetailsCard({ title, value }: { title: string; value: unknown }) {
    return (
        <Card>
            ...
            <CardContent>
                <p className="whitespace-pre-wrap text-sm leading-6 text-[var(--color-text)]">{formatValue(value)}</p>
            </CardContent>
        </Card>
    )
}
...
function getPersonName(record: AdminCrudRecord): string {
    return [record.last_name, record.first_name, record.middle_name].filter(Boolean).map(String).join(' ')
}
