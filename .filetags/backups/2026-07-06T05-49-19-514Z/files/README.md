...
`008` остаётся историческим маркером в `schema_migrations`. Сам рефакторинг учебной структуры
выполняет guarded schema repair: он проверяет наличие legacy-колонок до обращения к ним,
переносит связи в `department_faculties`, а затем атомарно перестраивает связанные таблицы.
На время rebuild внешние ключи отключаются вне транзакции, а перед commit выполняется
`foreign_key_check`. При дублях кафедр или названий специальностей внутри факультета запуск
останавливается с понятным сообщением и ID конфликтующих записей.
...
`academic_vacations`, если она отсутствует. Legacy rebuild сохраняет записи и ID, выполняется
в одной транзакции и откатывается целиком при ошибке. Затем schema validation сверяет все
таблицы и колонки из `adminCrudEntities` и выдаёт ошибку с точным именем отсутствующей
таблицы/колонки.
